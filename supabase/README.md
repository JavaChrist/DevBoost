...
| `003_avatars.sql` | Bucket Storage public `avatars` + policies RLS (lecture publique, écriture owner-only dans `{user_id}/`) |
| `004_subscriptions.sql` | Tables `subscriptions` + `payment_events` (Mollie), RPC `start_free_trial()` et helper `is_premium(uid)`, RLS owner-only en lecture, écritures réservées aux Edge Functions |
